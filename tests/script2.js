...
function spawn() {
    ...
    // apply some rotation
    dr = 20;//(-0.5 + Math.random()) * 16 * Math.PI;
}
...
function update(dt) {
    ...
    var spinCoef = 0.00001;
    let force = (spinCoef * ((size * dr / Math.sqrt(velsq))*0.5)) * velsq * size*size;//(spinCoef * velocity * circumference * circumference * dr) / (2 * size);
